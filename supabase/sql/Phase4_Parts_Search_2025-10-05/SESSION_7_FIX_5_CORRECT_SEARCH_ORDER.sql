-- ============================================================================
-- SESSION 7 - FIX 5: Rebuild Search Function with CORRECT Cascade Order
-- Date: 2025-10-05
-- Purpose: Fix search cascade order - FAMILY→PART→OEM→MAKE→MODEL→YEAR
-- ============================================================================

-- CRITICAL: Current function has WRONG order (MAKE first, PART last)
-- CORRECT order: Filter by PART first, then narrow by car details
...
BEGIN
    ...
    IF (part_param IS NULL OR part_param = '') AND (free_query_param IS NULL OR free_query_param = '') THEN
        RETURN; -- No part search criteria
    END IF;
    
    -- ============================================================================
    -- STEP 1: FAMILY (highest priority filter)
    -- ============================================================================
    ...
    IF family_param IS NOT NULL AND family_param != '' THEN
        ...
        -- If no results, remove family filter
    ...
    -- ============================================================================
    -- STEP 2: PART SEARCH (free_query OR part_param) - WITH NORMALIZATION
    -- ============================================================================
    ...
    IF make_param IS NOT NULL AND make_param != '' THEN
        ...
        -- If no make results, remove filter
        IF result_count = 0 THEN
...
SELECT 'Test: Corrected cascade order' as test;
