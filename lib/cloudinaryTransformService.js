...
export function generateTransformationUrl(supabaseUrl, options = {}) {
  ...
  const transformations = [
    ...
    // Business name (top text - lighter blue)
    `co_rgb:1e40af,l_text:Palatino_22_bold_italic_left:${encodedName}`,
    'fl_layer_apply,g_south_east,x_30,y_90',

    // Business license (lighter blue)
    `co_rgb:1e40af,l_text:Palatino_20_italic_left:${encodedLicence}`,
    'fl_layer_apply,g_south_east,x_30,y_70',

    // Plate label + actual plate number (bright red/orange)
    `co_rgb:dc2626,l_text:palatino_20_italic_left:${encodedPlateLabel}${encodedPlate ? '%3A%20' + encodedPlate : ''}`,
