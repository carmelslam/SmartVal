  ...
  auth: {
    ...
    setSession: async ({ access_token, refresh_token }) => {
      try {
        ...
        // For recovery tokens, we need to verify them using the token endpoint
        // Recovery tokens are NOT JWT access tokens - they're one-time use tokens
        const url = `${supabaseUrl}/auth/v1/token?grant_type=recovery`;
        ...
        if (!response.ok) {
          const errorData = await response.json();
          console.error('❌ Set session error:', errorData);
          return { 
            data: { user: null, session: null }, 
            error: { 
