...
class FileUploadService {
    ...
    async uploadToSupabase(file, uploadOptions = {}) {
        ...
        try {
            ...
            const documentData = {
                case_id: caseId,
                category: 'image', // Always 'image' for documents table (not the form's category)
                ...
                file_metadata: {
                    ...
                    image_category: category, // Store form's category in metadata for reference
                    ...metadata
                },
    ...
    /**
     * Log file operation
     * NOTE: Temporarily disabled due to VOID function JSON parsing error
     * TODO: Fix when audit_log is properly configured
     */
    async logFileOperation(documentId, operation, details = {}) {
        // Temporarily disabled - VOID RPC functions cause JSON parsing errors
        // Images are still tracked in images table and documents table
        return;
